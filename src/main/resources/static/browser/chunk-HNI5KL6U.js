...
var ModalService = class _ModalService {
  constructor() {
    this.modalState = new BehaviorSubject({
      isOpen: false,
      config: null,
      resolve: null
    });
...
var AuthService = class _AuthService {
  constructor(http, router, modalService, platformId) {
    this.http = http;
    this.router = router;
    ...
    this.INACTIVITY_TIME = 20 * 60 * 1e3;
    this.WARNING_TIME = 18 * 60 * 1e3;
  ...
  login(credentials) {
    return this.http.post(`${environment.apiUrl}/api/auth/login`, credentials).pipe(tap((response) => {
  ...
  logout() {
    ...
    this.modalService.confirm({
      ...
      message: "Has cerrado sesi\xF3n exitosamente. Se han limpiado todos los datos de la sesi\xF3n.",
      type: "info",
  ...
  clearAllServiceStates() {
    try {
      if (typeof window !== "undefined" && window.notificacionService) {
        window.notificacionService.limpiarEstado();
      }
      if (typeof window !== "undefined" && window.bandejaTramitesService) {
        window.bandejaTramitesService.clearTramites();
      }
    } catch (error) {
    }
  }
  ...
  isAuthenticated() {
    ...
    try {
      ...
      const isValid = expirationDate > /* @__PURE__ */ new Date();
      return isValid;
    } catch (error) {
      return false;
    }
  ...
  showSessionWarning() {
    return __async(this, null, function* () {
      const result = yield this.modalService.confirm({
        title: "Sesi\xF3n por expirar",
        message: "Tu sesi\xF3n expirar\xE1 en 2 minutos por inactividad. \xBFDeseas continuar?",
        type: "warning",
        confirmText: "Continuar",
        cancelText: "Cerrar sesi\xF3n",
        icon: "fas fa-clock"
      });
      if (result) {
  ...
  changePassword(request) {
    return this.http.put(`${environment.apiUrl}/api/auth/change-password`, request).pipe(catchError((error) => {
      return throwError(() => error);
    }));
  }
  getAdministrativosDisponibles() {
    return this.http.get(`${environment.apiUrl}/api/usuarios/administrativos-disponibles`).pipe(catchError((error) => {
      return throwError(() => error);
    }));
  }
  static {
    this.\u0275fac = function AuthService_Factory(t) {
      return new (t || _AuthService)(\u0275\u0275inject(HttpClient), \u0275\u0275inject(Router), \u0275\u0275inject(ModalService), \u0275\u0275inject(PLATFORM_ID));
    };
  }
  static {
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-HNI5KL6U.js.map
